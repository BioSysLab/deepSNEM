...
parser.add_argument('--batch_size', metavar='BS',
...
prot_embs, global_dict = load_prot_embs(
    SIZE, norm=False) if TYPE == 'seqveq' else load_prot_embs_go(SIZE, norm=False)
if TYPE == 'random':
    prot_embs = None
...
optimizer_sparse = torch.optim.SparseAdam(
    model.encoder.emb_layer.parameters(), lr=lr_sparse)
optimizer_dense = torch.optim.AdamW(list(model.parameters())[1:], lr=lr)

scheduler_dense = torch.optim.lr_scheduler.StepLR(
    optimizer_dense, step_size=4, gamma=0.05)
scheduler_sparse = torch.optim.lr_scheduler.StepLR(
    optimizer_sparse, step_size=4, gamma=0.05)
...
for epoch in range(EPOCHS):
    ...
    train_data_iterator = tqdm(train_loader, leave=True, unit='batch', postfix={
                               'Epoch': epoch+1, 'MI': '%.4f' % 0.0, })
    train(epoch)
