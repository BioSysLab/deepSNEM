...
def wcsv2graph(fname, global_dict, y):
    ...
    data_2.edge_index = negative_sampling(data_2.edge_index,
                                          num_neg_samples=800)
    ...
    data.acts = torch.cat(
        [data.downacts.float(), data.upacts.float()], dim=-1).double()
...
class SNDatasetSup(Dataset):
    """ Supervised Dataset PyTorch object."""
    def __init__(self, fnames, global_dict, y):
        super(SNDatasetSup, self).__init__()
        self.fnames = fnames
        self.gd = global_dict
        self.y = y
    ...
    def get(self, idx):
        data = ucsv2graph_infomax(self.fnames[idx], self.gd, y=self.y[idx])
        return data
